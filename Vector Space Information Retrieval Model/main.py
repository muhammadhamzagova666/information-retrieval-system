...
class GUI(QWidget):
    """
    A graphical user interface for the Vector Space Information Retrieval Model.

    Args:
        vector_space_model (VectorSpaceModel): An instance of the VectorSpaceModel class.
        research_papers (dict): A dictionary containing research papers as keys and their contents as values.

    Attributes:
        vector_space_model (VectorSpaceModel): An instance of the VectorSpaceModel class.
        research_papers (dict): A dictionary containing research papers as keys and their contents as values.
        search_field (QLineEdit): The input field for entering the search query.
        search_button (QPushButton): The button for initiating the search.
        tree_view (QTreeView): The tree view for displaying the search results.
    ...
    def init_ui(self):
        """
        Initialize the user interface by creating the layout and setting window properties.
    ...
    def search(self):
        """
        Handle the search process when the search button is clicked.
